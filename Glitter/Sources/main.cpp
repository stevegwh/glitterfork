...
int main(int argc, char *argv[])
{
    ...
    const std::string BINARY_PATH = fullPath.substr(0, fullPath.find_last_of('/', fullPath.size())) + "/";
    ...
    auto texture1 = LoadTexture(std::string(BINARY_PATH + "resources/container.jpg"), GL_TEXTURE0, GL_RGB);
    auto texture2 = LoadTexture(std::string(BINARY_PATH + "resources/awesomeface.png"), GL_TEXTURE1, GL_RGBA);
    
    sage::Shader shader(std::string(BINARY_PATH + "Shaders/shader.vert"), std::string(BINARY_PATH + "Shaders/shader.frag"));
    const char* vertexShaderSourceC = shader.vertexShader.c_str();
    const char* fragmentShaderSourceC = shader.fragShader.c_str();
    ...
    glShaderSource(fragmentShader, 1, shader.fragShader.c_str(), NULL);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
            glm::mat4 model = glm::translate(glm::mat4(1.0f), cube);
            model = glm::rotate(model, glm::radians(std::fmod((float) (100.0f * glfwGetTime()), 360.0f)), glm::vec3(1.0f, 0.0f, 0.0f));
